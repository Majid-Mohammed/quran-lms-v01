...
@section('title', 'Branch Deshboard')
...
@section('content')
    ...
    <div class="col-12 col-lg-8 main-content-wrapper ">   
        ...
        <form id="branchUpdateForm" action="{{ route('branch.update', $branch)}}" method="POST">
            @csrf
            @method('PUT')

            {{-- أزرار التحكم --}}
            <div class="d-flex sticky-top justify-content-start mb-3 d-print-none" style="top: 70px; z-index: 120;">
                <div id="viewModeButtons" class="d-inline p-1">
                    <button type="button" onclick="window.print()" class="btn btn-primary shadow-sm mt-1">
                        <i class="bi bi-printer-fill "></i> طباعة
                    </button>
                    <button type="button" onclick="toggleEditMode(true)" class="btn btn-primary shadow-sm mt-1">
                        <i class="bi bi-pencil-square "></i> تعديل البيانات
                    </button>
                    <button type="button" onclick="window.location.href='{{ route('branch.destroy', $branch) }}'" class="btn btn-danger shadow-sm mt-1">
            ...
            <div class="card border-0 shadow-lg rounded-4 overflow-hidden print-container report-card">
                ...
                <div class="card-body p-4 bg-white">
                    <div class="details-section p-4 rounded-3 mb-4" dir="rtl">
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-6 border-end border-dark">
                                <label class="text-muted small fw-semibold d-block mb-1"><i class="bi bi-pin-map"></i> العنوان :</label>
                                <span class="text-dark view-field ps-4">{{ $branch->address }}</span>
                                <input type="text" name="address" class="form-control edit-field d-none" value="{{ $branch->address }}">
                            </div>

                            {{-- المدينة --}}
                            <div class="col-6">
                                <label class="text-muted small fw-semibold d-block mb-1"><i class="bi bi-geo-alt"></i> المدينة :</label>
                                <span class="text-dark view-field ps-4">{{ $branch->city }}</span>
                                <input type="text" name="city" class="form-control edit-field d-none" value="{{ $branch->city }}">
                            </div>

                            {{-- رقم الهاتف --}}
                            <div class="col-6 border-end border-dark">
                                <label class="text-muted small fw-semibold d-block mb-1"><i class="bi bi-phone-fill"></i> رقم الهاتف : </label>
                                <span class="text-dark text-ltr d-block view-field ps-4">{{ $branch->phone }}</span>
                                <input type="text" name="phone" class="form-control edit-field d-none text-ltr" value="{{ $branch->phone }}">
                            </div>

                            {{-- السعة --}}
                            <div class="col-6">
                                <label class="text-muted small fw-semibold d-block mb-1"><i class="bi bi-layers-fill"></i> السعة الاستيعابية :</label>
                                <span class="text-dark view-field ps-4">{{ $branch->capacity }} طالب/طالبة</span>
                    ...
                    <div class="content-area py-3 d-none">
